...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Convert model score jsons to html table view and create graphs."
    )
    parser.add_argument(
        "scores_json_files", type=str, nargs="+", help="response score json files"
    )
    ...
    for metric_name, aggregate_key, score_key, chart_title in metrics:
        ...
        for model_data in data:
            if score_key == "sem_score_match_stats":
                ...
                values.extend(
                    [
                        info[score_key]["normalized_kendall_tau"]
                        for filename, info in model_data["scores"].items()
                    ]
                )
            else:
                values.extend(
                    [info[score_key] for filename, info in model_data["scores"].items()]
                )
...
def generate_detailed_model_scores(model_data):
    details_list = []
    for filename, info in model_data["scores"].items():
        matches_unpaired = json.dumps(
            {
                "Matches": info["sem_score_matches"],
                "Unpaired": info["unpaired_responses"],
            },
            indent=4,
        ).replace("\n", "<br>")
    ...
    html_content += (
        f'<h4>Prompt: <code class="prompt">{model_data["prompt"]}</code></h4>'
    )
    ...
    for header in [
        "Image Filename",
        "Final Score",
        "Matched Name Avg SemScore",
        "Count Accuracy",
        "Order Accuracy",
    ]:
        html_content = html_content.replace(
            f"<th>{header}</th>", f'<th class="sortable">{header}</th>'
        )
...
def generate_histogram(score_data, chart_title, limits, max_frequencies, score_key):
    ...
    return plot_histograms_svg(
        score_data, chart_title, "Score", "Frequency", xlim, ylim
    )
...
def generate_model_overview(model_data, metrics, limits, max_frequencies):
    ...
    for metric_name, aggregate_key, score_key, chart_title in metrics:
        ...
        if score_data:
            score_svg = generate_histogram(
                score_data, chart_title, limits, max_frequencies, score_key
            )
...
def generate_html(data):
    ...
    metrics = [
        ...
        (
            "Sem Score Match Stats",
            "matched_name_semantic_score_stats",
            "sem_score_match_stats",
            "Sem Score Match Stats Distribution",
        ),
        (
            "Order Accuracy",
            "order_accuracy_score_stats",
            "order_accuracy",
            "Order Accuracy Distribution",
        ),
        (
            "Count Accuracy",
            "count_accuracy_score_stats",
            "count_accuracy",
    ...
    for model_data in data:
        html_content += generate_model_overview(
            model_data, metrics, limits, max_frequencies
        )
        details_sections += "<hr>"
