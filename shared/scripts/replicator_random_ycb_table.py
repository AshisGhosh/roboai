...
with rep.new_layer():
    ...
    def randomize_objects():
        cracker_box = rep.create.from_usd(CRACKER_BOX, semantics=[("class", "red CheezIt cracker box")])
        cracker_plane_samp = rep.create.plane(scale=1, position=(0, 0, 0.1), rotation=(90, 0, 0), visible=False)

        sugar_box = rep.create.from_usd(SUGAR_BOX, semantics=[("class", "yellow and white Domino sugar box")])
        sugar_plane_samp = rep.create.plane(scale=1, position=(0, 0, 0.085), rotation=(90, 0, 0), visible=False)

        soup_can = rep.create.from_usd(SOUP_CAN, semantics=[("class", "red and white Campbells soup can")])
        soup_plane_samp = rep.create.plane(scale=1, position=(0, 0, 0.05), rotation=(90, 0, 0), visible=False)

        mustard_bottle = rep.create.from_usd(MUSTARD_BOTTLE, semantics=[("class", "yellow Frenchs mustard bottle")])
