...
class ImageProcessor(Node):
    ...
    def handle_response(self, response, cv_rgb_image, cv_depth_image):
        try:
            ...
            if len(grasps):
                self.get_logger().info(f"Received grasp poses: {[(grasp['cls_name'], round(grasp['obj'],2)) for grasp in response['result']]}")
                self.publish_grasp_image(grasps, cv_rgb_image)
                self.publish_grasp_markers(grasps, cv_depth_image)
        except Exception as e:
            self.get_logger().warn(f"Failed to receive valid response or grasp poses: {e}")
    ...
    def publish_grasp_image(self, grasps, original_image):
        for grasp in grasps:
            ...
            score = grasp['obj']
            label_with_score = f"{class_label} ({score:.2f})"  # Formatting score to 2 decimal places
            label_position = (points[0][0][0], points[0][0][1] - 10)
            cv2.putText(original_image, label_with_score, label_position, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    ...
    def publish_grasp_markers(self, grasps, cv_depth_image):
        ...
        for grasp in grasps:
            ...
            for pt in grasp_points:
                point_3d = self.project_to_3d(pt[0], pt[1], cv_depth_image[pt[1], pt[0]])
                if point_3d:
                    marker.points.append(Point(x=point_3d[0], y=point_3d[1], z=point_3d[2]))
