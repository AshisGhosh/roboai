...
from grasp_det_seg.utils.misc import config_to_string, norm_act_from_config, freeze_params
        ...
        for cls in cls_labels:
            ...
            for bbx_pred_i, cls_pred_i, obj_pred_i in zip(bbx_pred, cls_pred, obj_pred):
                ...
                if obj_pred_i.item() > threshold:
                    ...
                    pts = np.array([[pt1[0], pt1[1]], [pt2[0], pt1[1]], [pt2[0], pt2[1]], [pt1[0], pt2[1]]])
                    cnt = np.array([(int(bbx_pred_i[0]) + int(bbx_pred_i[2])) / 2,
                                        (int(bbx_pred_i[1]) + int(bbx_pred_i[3])) / 2])
            ...
            if bbox_best is not None:
                res = {"cls": cls, "obj": best_confidence, "bbox": bbox_best, "r_bbox": r_bbox_best}
                cnt = np.array([(int(bbox_best[0]) + int(bbox_best[2])) / 2,
                                        (int(bbox_best[1]) + int(bbox_best[3])) / 2])
                print(f"res {cls_list[cls]} | {cls_list[sem_pred[int(cnt[1]), int(cnt[0])]]}: {res}")
                output.append(res)
                pt1 = (int(bbox_best[0]), int(bbox_best[1]))
                pt2 = (int(bbox_best[2]), int(bbox_best[3]))
                # cv2.rectangle(img, pt1, pt2, (0, 255, 0), 2)
                cv2.putText(img_mask, cls_list[cls], (int(bbox_best[0]), int(bbox_best[1])), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
                if r_bbox_best is not None:
                    cv2.line(img_mask, tuple(r_bbox_best[0]), tuple(r_bbox_best[1]), (255, 0, 0), 2)
                    cv2.line(img_mask, tuple(r_bbox_best[1]), tuple(r_bbox_best[2]), (0, 0, 255), 2)
                    cv2.line(img_mask, tuple(r_bbox_best[2]), tuple(r_bbox_best[3]), (255, 0, 0), 2)
                    cv2.line(img_mask, tuple(r_bbox_best[3]), tuple(r_bbox_best[0]), (0, 0, 255), 2)
...
class GraspServer:
    ...
    def test_detect(self):
        return self.detect_from_path("/app/data/OCID_grasp/ARID20/table/top/seq08/rgb/result_2018-08-21-14-44-31.png")
...
if __name__ == "__main__":
    ...
    weights_path = "/app/data/weights/model_last.pth.tar"
    log_debug("Loading snapshot from %s", weights_path)
    snapshot = resume_from_snapshot(model, weights_path, ["body", "rpn_head", "roi_head", "sem_head"])
